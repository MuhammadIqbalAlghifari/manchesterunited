...
const NavbarLoggedInShop = () => {
  ...
  // Calculate the total quantity of all products in the "shop" cart
  const totalShopQuantity = state.shopCart.reduce((total, item) => total + item.quantity, 0);
            ...
            <nav className="fixed w-full shadow z-30 bg-[#121212] bg-opacity-70 tersembunyi" style={{backdropFilter: 'blur(10px)'}}>
                <div className="justify-between px-4 mx-auto lg:max-w-8xl md:items-center md:flex md:px-8">
                    <div>
                        <div className="flex items-center justify-between py-3 md:py-5 md:block">
                            ...
                            <div className="md:hidden flex items-center gap-1.5">
                                ...
                                <button className="text-gray-700 rounded-md outline-none relative" id="shopping-cart-list" onClick={() => toggleSidebar("shop")}>
                                    {totalShopQuantity > 0 && <span className="counter text-white bg-red-600 rounded-full p-1 absolute text-[8px] top-0 right-0">{totalShopQuantity}</span>}
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-7 h-7 text-white">
                    ...
                    <div className="hidden space-x-2 md:flex md:justify-center md:items-center md:gap-3">
                        ...
                        <button id="shopping-cart-list" className="relative" onClick={() => toggleSidebar("shop")}>
                            {totalShopQuantity > 0 && <span className="counter text-white bg-red-600 rounded-full p-1 absolute text-[9px] top-0 right-0">{totalShopQuantity}</span>}
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="lg:w-8 lg:h-8 md:w-6 md:h-6 w-4 h-4 text-white">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                            </svg>
                        </button>
